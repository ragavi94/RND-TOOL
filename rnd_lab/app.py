...
def update_topology_add_intf():
    ...
    if request_json['device']['interface'] != "":
        ...
        if flag:
            ...
            running_config['device'][device_index]['interface'].append(request_json['device']['interface'])  ##add new device interface to the exisitng json structure
            update_utility(running_config)
            return json.dumps({'topology':"Interface added in /etc/rnd_lab/topology_config.json"},indent=4),201
